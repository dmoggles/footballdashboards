...
class RollingNPxGDashboard(Dashboard):
    ...
    def _set_x_ticks(self, ax, data):
        ...
        ax.set_xticklabels(data["date"].apply(lambda x: x.strftime('%Y-%m-%d')).values, rotation=45, fontproperties=font_italic.prop, size=12)
    ...
    def _set_opposition_images(self, ax, data):
        ...
        for x_d,opp_name in zip(data["round"].values, data['opponent']):
            ax_1 = ax.inset_axes((x_d-0.3, ax.get_ylim()[0]+0.05, 0.6, 0.6/(x_size)*y_size), transform=ax.transData)
            try:
                ax_1.imshow(McLachBotBadgeService().team_badge(league, opp_name))
            except:
                ax_1.text(0.5, 0.5, '\n'.join(opp_name.replace('_',' ').title()), fontproperties=font_bold.prop, size=8, ha='center', va='center', color=self.textcolor)
            ax_1.axis('off')
    ...
    def _plot_title(self, data, ax:Axes):
        ...
        ax.text(0.1, 0.8, title, fontproperties=font_bold.prop, size=20, ha='left', va='top', transform=ax.transAxes, color=self.textcolor)
        subtitle = f"{season} season - {rolling_window} game rolling window"
        ax.text(0.1, 0.2, subtitle, fontproperties=font_italic.prop, size=12, ha='left', va='bottom', transform=ax.transAxes, color=self.textcolor)
        if normalized:
            ax.text(0.9, 0.2, "(NPxG normalized for opposition quality.)", size=10, ha='right', va='bottom', transform=ax.transAxes, fontproperties=font_italic.prop, color=self.secondary_textcolor)
    ...
    def _add_watermark(self, ax:Axes):
        ...
        if self.watermark_image:
            try:
                ...
                water_mark_ax = ax.inset_axes((1.0-0.05*ratio-0.005, 1-0.055, 0.05*ratio, 0.05), transform=ax.transAxes)
                ...
                pass
        if self.watermark:
            ax.text(0.99-dislocate_text, 0.985, self.watermark, size=8, ha='right', va='top', transform=ax.transAxes, fontproperties=font_italic.prop, color=self.secondary_textcolor)
