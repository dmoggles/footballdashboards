...
from footballdashboards.helpers.pitch_helpers import draw_passes_on_axes, draw_pass_legend_on_axes, plot_positional_heatmap_on_pitch, draw_defensive_events_on_axes, draw_convex_hull_without_outliers_on_axes, draw_defensive_event_legend
...
class MatchDashboard(Dashboard):
    ...
    def _draw_header(self, ax, data):
        ...
        formations = [non_carry.iloc[0]["formation"], non_carry.iloc[0]["starting_opponent_formation"]]
        
        match_dashboard_header(ax,match_date, league, teams, team_image_names, scores, formations, self.linecolor, self.facecolor, self.secondary_textcolor, home_away)
...
class PlayerMatchPassingDashboard(MatchDashboard):
    ...
    def _plot_pitches(self, data: pd.DataFrame, pitch: Pitch, axes: Dict[str, Axes]) -> None:
        ...
        for i, name in enumerate(names):
            ...
            draw_passes_on_axes(
                axes["pitch"][(r_i, r_j)], player_data, pitch
            )
            self._player_name_and_info(
                axes["pitch"][(r_i, r_j)], player_data, self.linecolor
            )

        plot_positional_heatmap_on_pitch(
            axes["pitch"][5][1], pitch, data, base_edge_color=self.linecolor, cmap=cmr.get_sub_cmap(cmr.amber_r, 0, 0.8),
        )
        axes["pitch"][5][1].text(
            0,
            107,
